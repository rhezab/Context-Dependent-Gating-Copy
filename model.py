...
class Model:
    ...
    def optimize(self):
        ...
        if par['gating_type'] is 'iXdG':
            ...
            # Make variables to record importance of each (hidden) unit
            # Make variables to record gates for each (hidden) layer, for each tasks
            self.importance = {}
            self.curr_gate = {}
            for layer in range(1, len(par['layer_dims'])-1):
                self.importance[layer] = tf.Variable(tf.zeros(par['layer_dims'][layer]), trainable=False)
                self.curr_gate[layer] = tf.Variable(tf.zeros(par['layer_dims'][layer]), trainable=False)
            self.gates = {}
            ...
            for task in range(par['n_tasks']):
                self.gates[task] = {}
                for layer in range(1, len(par['layer_dims'])-1):
                    self.gates[task][layer] = tf.Variable(tf.zeros(par['layer_dims'][layer]), trainable=False)
                    if task == 0:
                        self.curr_gate[layer] = tf.Variable(tf.zeros(par['layer_dims'][layer]), trainable=False)
            """

            # Build the sub-graph to compute importance
            self.compute_importance()

            # Build the sub-graph to create gates via importance for each task
            self.create_gates()
    ...
    def compute_importance(self):
        compute_importance_ops = []
        for layer in range(1, len(par['layer_dims'])-1):
            this_layer_ops = []
            weights_in = self.big_omega_var['layer'+str(layer-1)+'/W']
            layer_importance = tf.Variable(tf.zeros(par['layer_dims'][layer]), trainable=False)
            for unit in range(par['layer_dims'][layer]):
                weights_in_unit = weights_in[:,unit]
                unit_importance = tf.reduce_sum(weights_in_unit)
                this_layer_ops.append(tf.assign(layer_importance[unit], unit_importance))
                compute_importance_ops.extend(this_layer_ops)
            with tf.control_dependencies(this_layer_ops):
                compute_importance_ops.append(tf.assign_add(self.importance[layer], layer_importance))
        self.update_importance = tf.group(*compute_importance_ops)

        """
        compute_importance_ops = []
        # update_importance_ops = []
        for layer in range(1, len(par['layer_dims'])-1):
            weights_in = self.big_omega_var['layer'+str(layer-1)+'/W']
            # weights_out = self.big_omega_var['layer'+str(layer)+'/W']
            layer_importance = tf.Variable(tf.zeros(par['layer_dims'][layer]), trainable=False)
            for unit in range(par['layer_dims'][layer]):
                weights_in_unit = weights_in[:,unit]
                # weights_out_unit = weights_out[unit,:]
                unit_importance = tf.reduce_sum(weights_in_unit) # + tf.reduce_sum(weights_out_unit)
                compute_importance_ops.append(tf.assign(layer_importance[unit], unit_importance))
            compute_importance_ops.append(tf.assign_add(self.importance[layer], layer_importance))
            # update_importance_ops.append(tf.assign_add(self.importance[layer], layer_importance))
        self.update_importance = tf.group(*compute_importance_ops)
        # self.compute_importance_vals = tf.group(*compute_importance_ops)
        # self.update_importance = tf.group(*update_importance_ops)
        """

    def create_gates(self):
        create_gates_ops = []
        for layer in range(1, len(par['layer_dims'])-1):
            num_units_in_layer = par['layer_dims'][layer]
            _, ranked_indices = tf.math.top_k(self.importance[layer], k=num_units_in_layer)
            expected_active_units = round((1 - par['gate_pct']) * num_units_in_layer)
            active_indices = tf.gather(ranked_indices, [i for i in range(num_units_in_layer - expected_active_units, num_units_in_layer)])
            values = tf.ones(tf.shape(active_indices))
            active_indices = tf.cast(tf.reshape(active_indices, [tf.shape(active_indices)[0], 1]), dtype=tf.int64)
            sparse_gate = tf.SparseTensor(active_indices, values, [num_units_in_layer])
            reordered_gate = tf.sparse_reorder(sparse_gate)
            layer_gate = tf.sparse_tensor_to_dense(reordered_gate)
            create_gates_ops.append(tf.assign(self.curr_gate[layer], layer_gate))
        self.update_gates = tf.group(*create_gates_ops)
    ...
    with tf.Session() as sess:
        ...
        for task in range(par['n_tasks']):
            ...
            if par['gating_type'] is 'iXdG':
                ...
                curr_task_gate = {}
                for layer in range(1, len(par['layer_dims'])-1):
                    copy = tf.Variable(tf.zeros(tf.shape(model.curr_gate[layer])), trainable=False)
                ...
                if task>0:
                    layer_test = 1
                    print('Gates equal?')
                    print(sess.run(tf.equal(model.gates[task][layer_test], model.gates[task-1][layer_test])).all())

                # Create dictionary of gating signals applied to each hidden layer for this task
                gating_dict = {k:v for k,v in zip(gating, list(model.gates[task].values()))}
            else:
            ...
            for test_task in range(task+1):
                ...
                if par['gating_type'] is 'iXdG':
                    gating_dict = {k:v for k,v in zip(gating, list(model.gates[test_task].values()))}
                else:
