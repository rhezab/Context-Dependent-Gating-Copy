...
class Model:
        ...
        for n in range(par['n_layers']-1):
            ...
            with tf.variable_scope('layer'+str(n)):
                ...
                if n < par['n_layers']-2:
                    ...
                    self.x = tf.nn.dropout(tf.nn.relu(tf.matmul(self.x,W) + r + b), self.droput_keep_pct) # affine transform, non-linearity, dropout
                    self.x = self.x*tf.tile(tf.reshape(self.gating[n],[1,par['layer_dims'][n+1]]),[par['batch_size'],1]) # gating hadamard prod
    ...
    def optimize(self):
        ...
        prev_weights = {} # dict of prev param vals
        reset_prev_vars_ops = [] # list of ops to assign current var val to prev_weights[n] val
        self.big_omega_var = {} # dict of importance matrix of value for each variable (matrix contains imp for each param)
        aux_losses = [] # list of stabilization losses for each task
        ...
        for var in self.variables_stabilization:
            ...
            aux_losses.append(par['omega_c']*tf.reduce_sum(tf.multiply(self.big_omega_var[n], tf.square(prev_weights[n] - var)))) # computing and appending stabilisation loss for curr task
        ...
        self.aux_loss = tf.add_n(aux_losses) # add_n adds all input tensors element-wise
        ...
        with tf.control_dependencies([self.task_loss, self.aux_loss]):
            ...
            # control_dependencies creates a dep btwn train_op and task_loss, aux_loss even tho there isn't an explicit edge
        ...
        """
        insert graph construction for gating via importance
        """
...
def main(save_fn, gpu_id=None):
    ...
    with tf.Session() as sess:
        ...
        for task in range(par['n_tasks']):
            ...
            """
            par['gating'][task] = sess.run(ops to return gating)

            Run subgraph to assign val to par['gating'][task]
